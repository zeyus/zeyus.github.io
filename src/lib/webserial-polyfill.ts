...
class UsbEndpointUnderlyingSource implements UnderlyingSource<Uint8Array> {
	 ...
	 *
	 * @param {ReadableStreamController<Uint8Array<ArrayBufferLike>>} controller
	 */
	pull(controller: ReadableStreamController<Uint8Array<ArrayBufferLike>>): void {
		(async (): Promise<void> => {
			let chunkSize;
...
export class SerialPort {
	 ...
	 */
	public constructor(device: USBDevice, polyfillOptions?: SerialPolyfillOptions) {
		...
		this.readable_ = null;
		this.writable_ = null;
		this.serialOptions_ = { baudRate: 9600 };
	}
...
class Serial {
	 ...
	 * ports.
	 */
	async getPorts(polyfillOptions?: SerialPolyfillOptions): Promise<SerialPort[]> {
		polyfillOptions = { ...kDefaultPolyfillOptions, ...polyfillOptions };
