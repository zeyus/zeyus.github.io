...
function memcpy(
	dst: ArrayBufferLike | Uint8Array<ArrayBufferLike>,
	dstOffset: number,
	src: ArrayBufferLike | Uint8Array<ArrayBufferLike>,
	srcOffset: number,
	len: number
): void {
...
export const WebSerial = class {
	...
	async dtr(assert: boolean) {
		if (assert === undefined) {
			throw new Error(
				'Pass either true or false to dtr() to turn the Data Terminal Ready signal on or off'
			);
		} else if (assert) {
	...
	isSerialPort(port: object): boolean {
		return (
			(typeof SerialPort !== 'undefined' && port instanceof SerialPort) || port instanceof SPFill
		);
	}
	 ...
	 * @return {Uint8Array} data
	 */
	readArrayBufferUntil(needle: string | number | Array<number> | Uint8Array): Uint8Array {
		// check argument
	...
	async rts(assert: boolean) {
		if (assert === undefined) {
			throw new Error(
				'Pass either true or false to rts() to turn the Request To Send signal on or off'
			);
		} else if (assert) {
	...
	async selectPort(...args: any[]) {
		...
		if (1 <= args.length) {
			if (Array.isArray(args[0])) {
				...
				filters = args[0];
			} else if (this.isSerialPort(args[0])) {
	 ...
	 */
	async write(
		out: string | number | Array<number> | ArrayBuffer | ArrayBufferView | DataView
	): Promise<boolean> {
