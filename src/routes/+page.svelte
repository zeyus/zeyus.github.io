<script lang="ts">
	...
	const shaderLimits = {
		...
		timeMultiplier: { min: 0.0001, max: 5.0, default: 0.2 },
	...
	metaCtx.setMetadata({
		title: 'home',
		description:
			'The official website of zeyus, a cognitive science researcher, software engineer, hacker, musician, and generalist. ' +
...
<div class="flex flex-col md:w-full md:flex-row md:items-start md:justify-around">
	...
	{#if showShader && !loading}
		<div class="mb-4 flex flex-col justify-center md:m-0 md:flex-row md:p-0">
			<div
				class="mb-4 flex justify-center self-center justify-self-center md:mb-4 md:justify-self-end"
			>
			...
			<div
				class="mb-4 ml-2 flex w-64 flex-col justify-center self-center md:mb-4 md:w-32 md:justify-self-start"
			>
				...
				<div class="flex justify-between">
					<!-- Hide button -->
					<Button
						class="mt-2 h-6 w-6 p-0"
						color="primary"
						outline={true}
						size="sm"
						onclick={() => (showShader = false)}
						aria-label="Hide shader"
						title="Hide shader"
					>
						<EyeOutline class="m-0 h-4 w-4 p-0" />
					</Button>
					...
					>
						<UndoOutline class="m-0 h-4 w-4 p-0" />
					</Button>
				</div>
			</div>
		</div>
	{:else}
		<div class="mb-4 flex justify-center md:mb-4 md:self-center md:justify-self-center">
			<!-- Show button -->
			<Button
				color="primary"
				outline={true}
				size="sm"
				onclick={() => (showShader = true)}
				title="Show shader"
				aria-label="Show shader"
				class="m-0 h-6 w-6 p-0"
			>
				<EyeOutline class="m-0 h-4 w-4 p-0" />
			</Button>
		</div>
