...
<div id="fixedNavWrapper" class="relative w-full h-11 sm:h-14 z-30">
	{/* @ts-ignore */ null}
	<Navbar  fluid={true} navContainerClass="flex-nowrap relative content-center h-11 sm:h-14 flex-row items-center" class="px-2 py-0 pt-0 mt-0 sm:px-4 h-11 sm:h-14 fixed w-full z-20 top-0 start-0 whitespace-nowrap border-b shadow border-gray-700 dark:border-gray-700">
			...
			<NavUl
				...
				classes={{ ul: "md:flex md:dark:divide-none md:dark:bg-transparent md:items-stretch md:h-full md:flex-row md:space-x-4 dark:md:border-0 md:justify-self-end md:border-0 md:w-full md:flex-nowrap items-center items-center" }}
				onclick={() => onMenuItemClick()}
			>
				{#each menuItems as item}
					<NavLi href={item.path} class="my-auto md:p-0 md:pt-4 md:border-none md:bg-transparent md:align-middle align-bottom md:flex md:self-center h-11 md:h-full" activeClass="active dark:text-primary-500 font-bold text-lg text-primary-500">{item.name}</NavLi>
				{/each}
			</NavUl>
	</Navbar>
</div>
