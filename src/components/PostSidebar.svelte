...
<Button hidden={drawerVisible} onclick={toggleSidebar} class="cursor-pointer focus:ring-transparent focus:ring-0 fixed start-0 top-1/2 whitespace-normal p-0 hover:bg-gray-100 dark:hover:bg-gray-600 my-0 rounded-none mx-0">
...
<Sidebar id="postSidebar" isOpen={sidebarUi.isOpen} activateClickOutside={activateClickOutside}
    ...
    classes={{ active: activeClass, nonactive: nonActiveClass, div: "p-0 m-0 rounded-lg dark:bg-zinc-800" }}
    activeUrl={page.url.pathname} breakpoint="2xl" position="fixed" backdrop={false}
    class="w-64 top-1/6 h-2/3 p-0 m-0 rounded-lg 2xl:top-30 dark:bg-zinc-800">
        ...
        <SidebarGroup class="dark:bg-none">
            {#each Object.entries(sidebarItemsGrouped) as [key, sidebarItems]}
                ...
                {#each sidebarItems as item}
                    <SidebarItem spanClass="basis-11/12 self-center" activeClass="bg-gradient-to-r from-zinc-800 via-75% via-black/[.15] to-zinc-800 current-post flex text-sm flex-row font-bold text-primary-500 ps-0" nonActiveClass="flex font-bold text-sm flex-row ps-0" href={item.path} label={item.props.short_title || item.props.title}>
                        {#snippet subtext()}
